...
const defaultTemplates = [
  {
    id: 'paper',
    name: '论文生成',
    description: '生成符合投稿结构的论文草稿',
    domain: 'writing',
    content: `## 角色
你是一名熟悉学术写作规范的研究者。

## 任务
生成一篇满足投稿要求的论文草稿，涵盖引言、相关工作、方法、实验、结论。
...
export const useTemplatesStore = defineStore('templates', {
  ...
  getters: {
    allTemplates: (state) => [
      ...state.defaultTemplates.map(t => ({ ...t, builtIn: true })),
      ...state.userTemplates.map(t => ({ ...t, builtIn: false }))
    ]
  },
  ...
  actions: {
    ...
    updateTemplate(id, updates) {
      ...
      if (idx !== -1) {
        this.userTemplates[idx] = { ...this.userTemplates[idx], ...updates }
      }
    },
